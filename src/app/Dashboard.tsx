    ...
    <div className="flex flex-col items-center justify-center min-h-full px-3 py-4 sm:p-6 md:p-8">
      ...
      <div className="w-full max-w-lg mb-4 sm:mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={signOut}
            className="p-2.5 rounded-lg bg-brand-card border border-brand-border hover:border-red-500/50 text-brand-muted hover:text-red-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
            aria-label="Sign out"
            title="Sign out"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {user?.email && (
        <div className="w-full max-w-lg mb-3">
          <p className="text-brand-muted text-[11px] text-center">
            Signed in as <span className="text-brand-text/70">{user.email}</span>
          </p>
        </div>
      )}
